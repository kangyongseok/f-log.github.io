...
```
위 코드는 단순히 사용자에게 입력받은 텍스트의 첫글자 주소를 s 에 저장하고 그 주소를 그대로 다시 포인터 t 로 받아온것이기때문에 s 와 t 가 바라보고있는 주소는 동일한 주소가 됩니다. 이건 복사라고 볼수 없습니다. 복사가되었다면 t 와 s 는 주소의 값이 바뀌었을때 서로 다른값을 가져야하지만
  
현재 같은주소를 바라보고있기때문에 값의 변경사항에 대해서 동일한 값을 출력할 뿐입니다.
...
int main(void) {
  char *s = get_string("s: "); // 사용자 입력받기
  char *t = malloc(strlen(s) + 1); // malloc 으로 메모리영역 할당

  for (int i = 0, n = strlen(s); i < n + 1; i++) { // 종단문자까지 루프
    t[i] = s[i];
  } // 대체가능 strcpy(t, s);
}
```
...
메모리를 할당하는 이유는 데이터를 저장하기위해 충분한 메모리공간을 미리 확보해두어 쓰레기값이 되지 않도록 하기 위함이며  
  
메모리 해제는 하는 이유는 더이상 사용하지않는 메모리를 제거하여 메모리여유공간을 확보하고 실행시간을 더 빠르게 하기 위함 입니다.
...
```
위의 코드는 얼핏보면 맞는것같지만 원하는 결과를 얻을수는 없습니다. `swap(int a, int b)` 는 단지 x 와 y 의 값을 복사해와 a 와 b 의 값을 바꾸고는 사라집니다. 즉 스왑은 정상적으로 동작했지만 x 와 y 의 값이 바뀐것은 아닙니다.  

이를 이해하기 위해서는 메모리의 스택영역와 힙 영역에 대한 이해가 필요합니다.
  
스택은 main 함수가 시작하면서부터 발생하는 함수와 지역변수들이 쌓이는 영역이고 heap 은 malloc 으로 할당된 메모리의 데이터가 저장되는 곳입니다.  

![메모리 영역의 스택과 힙](https://cs50.harvard.edu/x/2020/notes/4/memory_layout.png)  

main 함수가 실행되면 x 와 y 의값이 스택에 먼저 쌓이고 그위에 a 와 b 의 변수값이 쌓이게됩니다. 물건이 쌓여있으면 가장 위에있는 즉 가장 나중에 들어온것부터 치워야 아래것을 사용할 수 있듯이 a, b는 값을 복사해와 먼저 실행되고 사라져버리고 전혀 값이 바뀌지않은 x 와 y 의 값만 남게 됩니다.  

해당 문제를 해결하고 정상적으로 원하는 x와 y의 스왑을 실행하려면 포인터를 사용하여 주소를받아와 실행해야합니다.
...
```
이렇게 포인터를 사용하게되면 a 와 b 는 단순히 값의 복사가 아닌 x 와 y를 참조하고있는 데이터가 되므로 정상적으로 데이터의 교환이 이루어집니다.
